// Mostly copied from: https://github.com/imgui-rs/imgui-rs/blob/main/imgui-winit-support/src/lib.rs
...
fn to_imgui_key(key: VirtualKeyCode) -> Option<Key> {
    match key {
...
fn handle_key_modifier(io: &mut Io, key: VirtualKeyCode, down: bool) {
    match key {
        ...
        VirtualKeyCode::LAlt | VirtualKeyCode::RAlt => io.add_key_event(imgui::Key::ModAlt, down),
        VirtualKeyCode::LWin | VirtualKeyCode::RWin => io.add_key_event(imgui::Key::ModSuper, down),
        _ => {}
    }
}
...
impl WinitPlatform {
    ...
    pub fn prepare_render(&mut self, ui: &Ui, window: &Arc<Window>) {
        let io = ui.io();
        if !io
            .config_flags
